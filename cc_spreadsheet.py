...
def get_sheet():
    ...
    # authenticate + obtain the cc spreadsheet URL
...
def get_json_lut(sheet: gspread.Spreadsheet):
    ...
    for worksheet in sheet.worksheets():
        ...
        compact_col_count = len(values[0]) if len(values) > 0 else 0  # worksheet.col_count returns all columns (even those with no data)
...
def write_json_lut(json_lut):
    ...
    with open('build/cc_spreadsheet.json', 'w') as file:
